    ...
    async execute(message, args, config, fs) {
        if (args.length > 2) {
            ...
            if (Number.isNaN(args[1]) || Number.isNaN(args[2])) {
                return message.channel.send(`<@${message.member.id}> Please provide existing catacombs levels`)
            }
            ...
            return message.channel.send(createSuccessEmbed(`<@${message.member.id}> It will take ${sum} catacombs xp to get from level ${args[1]} to level ${args[2]}`))
            ...
            message.channel.send(
                ...
                .then(async newMessage => {
                    ...
                    if (response === "Api throttle") { return message.edit("Api throttle") }
                    let cataXp = -1;
            ...
            return message.channel.send(`<@${message.member.id}> Please provide at least one catacombs level`)
        }
    },
};
