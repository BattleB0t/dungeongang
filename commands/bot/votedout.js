...
module.exports = {
    ...
    async execute(message, args, config, fs) {
        ...
        if(args[1] == 'add'){
            ...
            if(uuid == 'invalid player') return message.channel.send('This player does not exist!')
            let jsonString = fs.readFileSync('./data/votedout.json')
            let json = JSON.parse(jsonString)
            if(json.users.includes(uuid)) return message.channel.send('This player is already on the voted-out list!')
        ...
        if(args[1] == 'remove'){
            ...
            let jsonString = fs.readFileSync('./data/votedout.json')
            let json = JSON.parse(jsonString)
            if(!json.users.includes(uuid)) return message.channel.send('This player is not on the voted-out list!')
